...
timeFramesEpoch = timeResolution*round(1/(group.procResult.tHRF(2)-group.procResult.tHRF(1))); % how many frames should be computed

%------define conditions
%please change according to your design
dataStat.condition.names = {'Upright' 'Inverted'}; % please change according to names, 
conditionROI = [1 2]; %only be able to analyse two conditions, hence if more than 2, please choose which two conditions
...
for groupCount=1:length(groupFinal)
    ...
    set(figureHbOHandle(groupCount,1),'YLim',[-.5E-5 .5E-5],'YTick',[-.5E-5 0 .5E-5]);
...
for groupCount=1:length(groupFinal)
    ...
    set(figureHbRHandle(groupCount,1),'YLim',[-.5E-5 .5E-5],'YTick',[-.5E-5 0 .5E-5]);
...
for groupCount=1:length(groupFinal)
    ...
    set(figureHbTHandle(groupCount,1),'YLim',[-.5E-5 .5E-5],'YTick',[-.5E-5 0 .5E-5]);
